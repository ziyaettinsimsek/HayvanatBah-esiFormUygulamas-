...
namespace HayvanatBahçesiFormUygulaması.UI
{
    public partial class Hayvanlar : Form
    {
        ...
        private void Tamambutonu_Click_1(object sender, EventArgs e)
        {
            DataGridViewRow row = dataGridView2.SelectedRows[0];
            Hayvan = new Hayvan()
            {
                ID = Guid.Parse(row.Cells[0].Value.ToString()),
                Turu = row.Cells[1].Value.ToString(),
                Kafesi = row.Cells[2].Value.ToString(),
                Yemi = row.Cells[3].Value.ToString(),
